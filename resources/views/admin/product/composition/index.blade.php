...
@section('title', __('Komposisi ' . $product->nama))
        ...
        <div class="card-header d-flex align-items-center">
            <div class="card-title mb-0 flex-grow-1">
                <div class="btn btn-primary">
                    <h6 class="mb-0 text-light">Daftar Komposisi {{ $product->nama }}</h6>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $("#table-data").DataTable({
                ...
                scrollX: false,
                ajax: {
                    url: `{{ route('admin.composition.index', ['product' => ':product_id']) }}`
        ...
        $('.btn_save').click(function(e) {
            ...
            var status;
            if (save_method == 'add') {
                url = `{{ route('admin.composition.store', ['product' => ':product_id']) }}`
                    .replace(':product_id', `{{ $product->id }}`);
                status = "Ditambahkan";
            } else {
                url =
                    `{{ route('admin.composition.update', ['product' => ':product_id', 'composition' => ':id']) }}`
                    .replace(':product_id', `{{ $product->id }}`)
                    .replace(':id', id_use);
                status = "Diubah";
        ...
        $(document).on('click', '.btn_edit', function() {
            ...
            $.ajax({
                url: `{{ route('admin.composition.edit', ['product' => ':product_id', 'composition' => ':id']) }}`
                    .replace(':product_id', `{{ $product->id }}`)
                    .replace(':id', id_use),
                type: 'GET',
                dataType: 'JSON',
        ...
        $(document).on('click', '.btn_delete', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ route('admin.composition.destroy', ['product' => ':product_id', 'composition' => ':id']) }}`
                            .replace(':product_id', `{{ $product->id }}`)
                            .replace(':id', id_use),
                        type: "POST",
                        dataType: "JSON",
