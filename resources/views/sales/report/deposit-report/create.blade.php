        ...
        <div class="card">
            <div class="card-header d-flex align-items-center">
                ...
                <a class="btn btn-lg btn-outline-primary rounded-pill btn-label mr-2"
                    href="{{ route('sales.deposit-report.index') }}">
                    Batal</a>
...
@section('script')
    <script>
        ...
        $.ajax({
            url: `{{ route('pharmacy.report', ':id') }}`.replace(':id', {{ $sales->id }}),
            type: "GET",
            dataType: "JSON",
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                window.location.href = '{{ route('sales.deposit-report.index') }}'
            }
        })
        ...
        function appendProduct(index, data = null) {
            let html = `
                    ...
                    <input type="hidden" value="${data?.price_stock}" class="form-control product_data product-price" data-id=${product_index} name="pharmacies[${index}][products][${product_index}][price]" id="pharmacies[${index}][products][${product_index}][price]">
        ...
        $(document).on('click', '.btn_submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `{{ route('sales.deposit-report.store') }}`,
                        type: "POST",
                        data: formData,
                        contentType: false,
                        processData: false,
